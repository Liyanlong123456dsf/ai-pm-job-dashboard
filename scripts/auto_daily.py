...
def run_login_check(account: str = ''):
    """执行登录检查（调用 login_check.py）； account 不为空时透传 --account"""
    script = SCRIPT_DIR / 'login_check.py'
    logger.info(f'🔑 开始登录检查{f" [账号={account}]" if account else ""}...')
    result = None
...
def run_daily_update(quick=False, account: str = ''):
    """以子进程方式运行 daily_update.py，带超时保护； account 不为空时透传"""
    ...
    logger.info(f'🚀 启动 daily_update.py [{mode_label}模式]{f" [账号={account}]" if account else ""} ({timeout_hint})')
...
def _child_env():
    ...
    env.setdefault('AI_PM_SHOW_PROGRESS_GUI', '0')  # UI 统一到 Electron
...
def _pid_is_running(pid):
    """判断给定 PID 是否还在运行（Windows 原生 API + Unix signal 0 双分支）"""
    if not pid:
        return False
    try:
        pid = int(pid)
    ...
    if sys.platform == 'win32':
        # Windows: 用 ctypes 调 OpenProcess + GetExitCodeProcess
        # os.kill(pid, 0) 在 Windows 上行为不可靠，会误判已死进程为存活
        try:
            import ctypes
...
def _select_account_and_login(round_num):
    """
    按账号池轮换逐个尝试登录检查，返回 (alias 或 '', login_ok, login_state)
    - alias='' 时表示使用默认 Profile（单账号降级）
    - 所有账号都失败时返回最后一个的 login_state
    """
    try:
        sys.path.insert(0, str(SCRIPT_DIR))
        import account_pool
    except Exception as e:
        logger.warning(f'账号池不可用，降级为单账号: {e}')
        login_ok, login_state = run_login_check()
        return '', login_ok, login_state

    cfg = account_pool.load_config()
    accounts = cfg['accounts']
    if len(accounts) < 2:
        # 只有 1 个账号：行为与旧版一致，不透传 --account，但仍更新池状态
        single_alias = accounts[0]['alias'] if accounts else ''
        login_ok, login_state = run_login_check(single_alias)
        return single_alias, login_ok, login_state

    # 多账号轮换
    tried = []
    last_state = {}
    for _ in range(len(accounts) + 1):
        acc = account_pool.pick_next_account(exclude=tried)
        if not acc:
            break
        alias = acc['alias']
        tried.append(alias)
        _write_runtime_state(round_num, 'login_check', f'检查账号 [{alias}]')
        login_ok, login_state = run_login_check(alias)
        last_state = login_state
        if login_ok:
            logger.info(f'✅ 账号 [{alias}] 登录有效，本轮使用此账号')
            return alias, True, login_state
        logger.warning(f'⚠️ 账号 [{alias}] 登录失败: {login_state.get("detail", "")[:100]}')

    # 所有账号都失败
    failed_accounts = account_pool.get_failed_accounts()
    if account_pool.should_send_all_failed_alert():
        logger.error(f'🚨 全部 {len(failed_accounts)} 个账号均失效，发送飞书告警')
        try:
            from feishu_alert import send_account_pool_alert
            send_account_pool_alert(failed_accounts, throttle_sec=0)
            account_pool.mark_alert_sent()
        except Exception as e:
            logger.warning(f'飞书告警发送异常: {e}')
    else:
        logger.info('🔕 打响节流未到，不重复发送飞书通知')
    return (tried[-1] if tried else ''), False, last_state
...
def _run_one_round(round_num):
    ...
    # ① 登录检查（账号池轮换）
    active_account, login_ok, login_state = _select_account_and_login(round_num)
    if not login_ok:
        # 所有账号都失败 → 使用账号池的全局冷却时间
        try:
            sys.path.insert(0, str(SCRIPT_DIR))
            import account_pool
            cfg = account_pool.load_config()
...
def main():
    ...
    os.environ.setdefault('AI_PM_SHOW_PROGRESS_GUI', '0')  # UI 统一到 Electron
